...
class SourceDataHandler:
    ...
    @functools.cache
    def _get_rollable_effects_wrapped(self, pool_type: Literal["normal", "deep", "curse"] = "normal") -> list[int]:
        pools: set[int] = set()
        relics = self._relic_table[self._relic_table.index.to_series().isin(self.get_safe_relic_ids())]
        match pool_type:
            case "normal":
    ...
    def get_rollable_effects(self, pool_type: Literal["normal", "deep", "curse"] = "normal"):
        """Get all effects that can roll (chanceWeight > 0) in a given pool type (normal, deep, or curse)."""
